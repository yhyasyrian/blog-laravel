<x-app-layout>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-12 w-full xl:container px-4 mx-auto mt-6 gap-6">
        <x-card-layout class="lg:col-span-9 p-6">
            <div class="img relative max-w-2xl mx-auto overflow-hidden rounded-md">
                <img src="{{asset($seo->image)}}" alt="{{$post->title}}"
                class="bord w-full rounded-md mb-6 object-cover aspect-video transition hover:scale-110 hover:rotate-3"
                >
            </div>
            <div class="post">
                {!! $post->body !!}
        ...
        <x-card-layout class="lg:col-span-3">
            ...
            <div class="flex flex-col gap-y-12">
                @foreach($postsRandom as $post)
                    <x-card :title="$post->title" :description="mb_substr(strip_tags($post->body),0,120).'...'" image="{{asset($post->photo())}}" :link="route('post',['slug'=>$post->slug()])" class="group w-[90%] mx-auto border" ></x-card>
                @endforeach
            </div>
        </x-card-layout>
    </section>
